...
double S=0, Y=0, WGain=0;
  ...
  double pv = pc_array[line_no_2].v_px;//pixel position
  double dx = pc_array[line_no_2].x_3d;
  double dy = pc_array[line_no_2].y_3d;
  double dz = pc_array[line_no_2].z_3d;//3D position
 ...
 for (int u=0; u<1836; u=u+1)
 {
    S=0; Y=0;
     for (int k=kin; k<sd; k=k+1)
     {
            ...
            if(pv <= v+dim[0]-grid) { kin=k; }
            if(pv >= v+dim[0]+grid) { break;  }        
        
        if ( pu > u-grid && pu< u+grid && pv > v+dim[0]-grid && pv < v+dim[0]+grid )
        {
            Gr = dx/mr;
